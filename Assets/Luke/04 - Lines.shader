    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float lines = frac(i.uv.y * _LineDensity) * _LineStrength;
                //return fixed4(col.rgb, jellyLines * _GlobalOpacity);
                //fixed4 col;
                //time addierenen dann frac

                float pos = i.uv.y - (_Time.y);
                float jellyLinesPos = jellyfish * (lines + frac(pos));
                jellyLinesPos = clamp(jellyLinesPos, 0.2, 1.0);
                fixed4 col = _BaseColor;
                return fixed4(col.rgb, jellyLinesPos);
            }
            ENDCG
        }
    }
